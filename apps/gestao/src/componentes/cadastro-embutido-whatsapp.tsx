...
import { concluirCadastroEmbutido, iniciarCadastroEmbutido } from '../app/implantacao/acoes';
...
const EVENTOS_SEM_SUPORTE = ['FINISH_ONLY_WABA', 'FINISH_OBO_MIGRATION', 'FINISH_GRANT_ONLY_API_ACCESS'];
...
function classificarEvento(dados: { event?: unknown; error_message?: string }): Classificacao {
  ...
  if (evento === 'CANCEL') {
    return dados.error_message ? { tipo: 'erro', mensagem: dados.error_message } : { tipo: 'cancelado' };
  }
  return { tipo: 'ignorar' };
}
...
function criarTratador(
  ...
  return (evento) => {
    ...
    try {
      const dados: unknown = typeof evento.data === 'string' ? JSON.parse(evento.data) : evento.data;
      if (dados && typeof dados === 'object' && (dados as { type?: string }).type === 'WA_EMBEDDED_SIGNUP') {
 ...
 */
function executarCadastro(appId: string, versao: string, configId: string): Promise<Credenciais | null> {
  return new Promise((resolver, rejeitar) => {
    ...
    const tratador = criarTratador((dados) => {
      ...
      if (resultado.tipo === 'fim') {
        ...
        encerrar(() =>
          rejeitar(new Error('O cadastro terminou sem um número da API do WhatsApp para conectar.')),
        );
  ...
  async function conectar(): Promise<void> {
    ...
    try {
      ...
      try {
        credenciais =
          inicio.modo === 'real'
            ? await executarCadastro(inicio.appId ?? '', inicio.versao ?? VERSAO_PADRAO, inicio.configId ?? '')
            : credenciaisDeEnsaio();
      } catch (erro) {
