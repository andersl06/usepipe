...
export default async function PaginaDoContato({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  if (!UUID.test(id)) notFound();

  const [casca, contato, fuso] = await Promise.all([
  ...
  return (
    <div className="pt-app">
      ...
      <BarraDoContato contato={contato} />
...
function porData(instante: Date | null, fuso: string): string {
  if (!instante) return '—';
