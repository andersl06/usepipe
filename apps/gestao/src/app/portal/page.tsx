...
import { carregarPortal, POR_PAGINA, type DadosDoPortal } from '../../lib/portal';
...
const URL_AJUDA = process.env['NEXT_PUBLIC_PIPE_AJUDA_URL'] ?? '';

type Fluxo = DadosDoPortal['fluxos'][number];
...
export default async function PaginaPortal({
  ...
  const pagina = Math.max(1, Number(primeiro(parametros['pagina'])) || 1);
  const porPagina = Number(primeiro(parametros['por'])) || POR_PAGINA[0];
  ...
  const dados = await carregarPortal({ busca, pagina, porPagina });
