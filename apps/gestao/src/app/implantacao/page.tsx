...
import { carregarImplantacao } from '../../lib/implantacao';
...
export default async function PaginaImplantacao() {
  ...
  const { sinais, canais } = await carregarImplantacao();
    ...
    <>
      ...
      <section className="card" id="whatsapp">
        ...
        <p className="sub">
          ...
          Manager dele. Ao fim, o Pipe troca o código pelo token, registra o número e aponta o webhook —
          ninguém copia URL nem token.
        </p>
                ...
                <article key={c.id} className="cartao-lista">
                  ...
                  <div className="cl-acoes">
                    <Etiqueta tom={precisaReconectar ? 'alerta' : 'sucesso'}>
                      {!c.ativo ? 'Desligado' : c.reautorizacaoPendente ? 'Reautorizar' : 'Conectado'}
                    </Etiqueta>
        ...
        <ConectarWhatsApp rotulo={canais.length === 0 ? 'Conectar WhatsApp' : 'Conectar outro número'} />
        <FormularioManual />
      </section>
      ...
      <section className="card" id="equipe">
        <h3>Equipe</h3>
        <p className="sub">
          Quem ainda não tem domínio verificado entra por convite: o link cria o acesso com o papel
          escolhido e liga a conta do Google na primeira entrada.{' '}
          {numero(sinais.membros)} pessoa(s) com acesso e {numero(sinais.convites)} convite(s) criado(s).
        </p>
        <FormularioConvite />
      </section>
      ...
      <section className="card" id="contatos">
        ...
        {ultima ? (
          <p className="note">
            ...
            {ultima.temFalhas ? (
              <a href={`/implantacao/importacoes/${ultima.id}/falhas`}>
