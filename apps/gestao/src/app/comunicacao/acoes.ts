...
import { CABECALHOS_TEMPLATE } from '../../lib/comunicacao';
...
function recarregar() {
  revalidatePath('/comunicacao/respostas-prontas');
  revalidatePath('/comunicacao/modelos');
}

// --------------------------------------------------------- respostas prontas

export async function salvarRespostaPronta(_anterior: Resultado, dados: FormData): Promise<Resultado> {
...
export async function salvarModelo(_anterior: Resultado, dados: FormData): Promise<Resultado> {
  ...
  if (!corpo) return falha('Cole o texto aprovado na Meta, para referência de quem vai usar o modelo.');

  let variaveis: string[];
  try {
    const bruto: unknown = JSON.parse(variaveisJson);
    if (!Array.isArray(bruto) || !bruto.every((v) => typeof v === 'string')) throw new Error('formato');
    variaveis = bruto;
  } catch {
  ...
  return consultar(async (tx) => {
    ...
    if (canalEscolhido.tipo !== 'whatsapp_cloud') return falha('Modelo de mensagem é só para canal WhatsApp.');
