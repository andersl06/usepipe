...
/**
 * Criar o fluxo — o clique no botão "Criar fluxo" do passo do nome.
 *
 * Na origem é a `createApplication()` do `CreateApplicationController` chamando
 * `CreateApplicationService.createApplication(application, 'builder')`. Os sete
 * passos desse serviço — e o que o fluxo faz A MAIS que o roteador neles —
 * estão comentados em `../gravar.ts`, que é o que grava para as duas telas.
 *
...
export async function criarFluxo(dados: FormData): Promise<void> {
  ...
  /* `goToApplicationDetails()` deles termina em
     `$state.go('auth.application.detail.home', { shortName })` — a tela do
     contato recém-criado, e não a lista de onde a pessoa veio. Essa tela já
     existe: `app/fluxo/[id]`.

     Vai pelo `id` e não pelo `short_name` porque a coluna ainda não tem índice
     único — a nota inteira está no cabeçalho daquela página.

     Detalhe que só vale para o fluxo: antes de sair, `goToApplicationDetails`
     dispara o rastro `create-chatbot-flowtest`, que o roteador PULA
     (`if (this.application.template !== Master)`). É o sinal de que o próximo
     gesto esperado de quem acabou de criar um fluxo é testar a conversa — o
     roteador não tem conversa para testar. */
