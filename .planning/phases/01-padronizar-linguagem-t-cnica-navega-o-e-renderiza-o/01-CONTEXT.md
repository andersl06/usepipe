...
- **D-42 (execução, 2026-09-24, decisão do dono no portão 1):** A tela de atendimento do Desk é a raiz do app: `/` (no futuro `<tenant>.desk.usepipe.ai/`, Phase 01.1), sem segmento `/chat`. As demais telas do Desk seguem `/contacts` e `/analytics`. O que o contrato de navegação (`std/nav-contract.md`) e D-27/D-32 dizem sobre `/chat` vale para `/`: conversa aberta em React state, voltar fecha o painel e mostra a lista. `/chat` e `/chat/:id` deixam de existir (corte seco, D-14). Painel/módulo principal do Portal/Gestão = `application`; componente de painel na UI = `panel`.
