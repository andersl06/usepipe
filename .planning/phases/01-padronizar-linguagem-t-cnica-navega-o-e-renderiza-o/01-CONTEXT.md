...
- **D-40-exec (execução, 2026-09-24, exceção ao D-37 aprovada pelo dono; citada como "D-40" em notas de triagem, SUMMARYs e prompts de execução):** Na execução da fase, os planos podem ser distribuídos entre 5 executores: Codex conta 1 (`~/.codex`), conta 2 (`~/.codex-conta2`), conta 3 (`~/.codex-conta3`), Sonnet e Haiku; até 3 Codex rodam em paralelo, todos em `gpt-5.6-sol` medium. O Codex pode executar planos inteiros com `-s danger-full-access` (o `workspace-write` é recusado pelo sandbox do Windows), sempre num git worktree isolado próprio, sem push, deploy, alteração de banco ou escrita fora do worktree. O Sonnet revisa 100% do diff do Codex antes do merge em `std/english-rename` (D-24). Planos que tocam dados reais (ex.: 01-34) continuam sem Codex. Propostas semânticas via `codex-run.sh` continuam `read-only`.
...
- **D-42 (execução, 2026-09-24, decisão do dono no portão 1):** A tela de atendimento do Desk é a raiz do app: `/` (no futuro `<tenant>.desk.usepipe.app/`, Phase 01.1), sem segmento `/chat`. As demais telas do Desk seguem `/contacts` e `/analytics`. O que o contrato de navegação (`std/nav-contract.md`) e D-27/D-32 dizem sobre `/chat` vale para `/`: conversa aberta em React state, voltar fecha o painel e mostra a lista. `/chat` e `/chat/:id` deixam de existir (corte seco, D-14). Painel/módulo principal do Portal/Gestão = `application`; componente de painel na UI = `panel`.
- **D-43 (2026-09-24, dono):** Não há usuários nem dados reais em produção (ninguém usou). A virada (01-33) pode reconstruir o ambiente da VPS do zero: sem backup de Postgres/Redis, sem drenar filas antigas (Redis pode ser zerado), sem reenvio de convites, sem compatibilidade com sessões/links antigos. Produção = a VPS atual (`pipe.144-217-164-204.sslip.io`) até existir domínio. Domínio futuro: `usepipe.app` (ainda não comprado) — vale para a Phase 01.1; nada na Phase 1 deve fixar `usepipe.com.br` como novo valor.
- **D-44 (execução, 2026-09-24, dono aprovou antecipar):** A triagem de comentários dos planos 01-15, 01-20, 01-37 e 01-27 (e crm/css) é feita ANTES das fatias, pelo Codex, em worktrees `cx/tri-*`, gravando as propostas em `std/comments/<scope>.csv` com nota "pre-triage D-40; needs slice-time confirmation" nas linhas sensíveis. Os identificadores citados nos comentários ficam com o nome antigo; um passo mecânico com o mapa aprovado os renomeia na aplicação. O executor de cada fatia NÃO refaz a triagem: confere só as linhas marcadas, aplica com `tools/std/apply-comments.ts` e roda o gate.
- **D-45 (2026-09-24, dono, antecipação do portão 2 — std/GATE2-PACKET-DRAFT.md):** (a) Os 24+ endpoints acessíveis por chave de API de cliente (`/v1/conversas*`, `/v1/contatos*`, `/v1/filas`, `/v1/atendentes*`, `/v1/mensagens-ativas*`, `/v1/anexos*`), marcados `keep` pelo 01-35, SÃO renomeados agora com os demais (sem cliente real, D-43); `std/wire-contracts.csv` passa essas linhas de `keep` para a decisão de rename correspondente. (b) Exceções do mapa de CSS aprovadas: painel→`panel`, papel→`paper`, estado→`status`. (c) Os 744 valores persistidos em `std/persisted.csv` ficam com o nome atual. O portão 2 (01-12) ainda aprova o mapa completo; estes três itens já não voltam como pergunta.
- **D-46 (2026-09-24, dono, portão 2):** Os apps/pacotes com nome em português são renomeados como propõe o mapa (01-11): `gestao-vite` → `management-vite`, `ponte` → `bridge` (e o nome de workspace correspondente). Inclui Dockerfiles, compose, labels do Traefik, caminhos na VPS e scripts de build; como a VPS é reconstruída do zero na virada (D-43), não há compatibilidade de caminhos antigos a manter.
- **D-47 (2026-09-25, dono, portão 2 aprovado — std/GATE2-REVIEW.md):** Mapa aprovado com 4 ajustes: (1) URLs públicas do site (`apps/site/blog/*` e `apps/site/ferramentas/*`, inclusive o segmento `ferramentas`) FICAM em português — são conteúdo indexado pelo Google para público brasileiro, não identificador técnico; só arquivos/símbolos internos do site renomeiam. (2) Amostra de nomes aprovada como está (management-vite, bridge, realtime, /updates, /opportunities, quality-review, returnTo/tab/queue/agent). (3) As ~900 linhas `test-title` propostas pelo 01-10 (api, infra, packages-db, workers, packages-contracts, packages-autenticacao, packages-armazenamento, packages-tempo-real) são RETRADUZIDAS antes de aplicar — ficam fora do conjunto aprovado até a retradução passar no check-map; as do 01-11 estão aprovadas. (4) D-29, D-31 e D-34 aprovados com as recomendações do pacote (D-34: nomes aprovados agora, validação ao vivo tela a tela durante as fatias).
- **D-48 (2026-09-25, dono):** Os 5.300 comentários já triados (D-44) são aplicados DE UMA VEZ, logo após o 01-13, num único passo com um único gate (`tools/std/apply-comments.ts` em todos os escopos), em vez de uma rodada por fatia. As tarefas de comentários de 01-15, 01-20, 01-27, 01-37 e 01-40 passam a ser só conferência das linhas marcadas "needs slice-time confirmation" no escopo da fatia. Identificadores citados dentro dos comentários mantêm o nome antigo até o 01-31, que roda um passe mecânico old→new (mapa aprovado) sobre o texto dos comentários.
- **D-49 (2026-09-25, dono, substitui as 5 fatias do D-19 na execução):** O mapa aprovado é aplicado DE UMA VEZ (símbolos, arquivos, literais, itens manuais dos planos de fatia, renomes de pacotes/apps/infra, comentários) num único worktree, com commit por passo e UM gate completo no fim, já que os três dry-runs passaram limpos em todos os escopos no portão 2. Ficam como passos separados depois da aplicação: CSS (01-25, assim que a ferramenta `rename-css` estiver pronta), navegação (01-22 e 01-24, mudança de comportamento), varredura final (01-31), revisão do dono (01-32), cutover (01-33) e histórico (01-41). As tags `std-slice-N-end` são substituídas por uma única `std-apply-all-end`.
- **D-40 (escopos; a D-40 original do planejamento, citada assim nos planos e em exceptions.csv):** Escopos de chave de API guardados no banco (ex.: `conversas:ler`) e códigos de erro persistidos são persistidos: vão para o inventário STD-06, não renomeiam nesta fase.
