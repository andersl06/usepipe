...
- **Idioma (revisado 24/09/2026, STD-08)**: Linguagem técnica (arquivos, pastas, funções, variáveis, types/interfaces/classes, controllers/services, rotas/endpoints, nomes de teste, comentários técnicos, novos contratos técnicos) em **inglês**, conforme `.planning/phases/01-padronizar-linguagem-t-cnica-navega-o-e-renderiza-o/std/CONVENTIONS-EN.md` e o glossário aprovado (`std/GLOSSARY.md`) — substitui a regra anterior de "tudo em português". Texto visível ao usuário (produto, UI, mensagens) continua em português/localizado e não entra nessa regra. Dados e contratos já persistidos (tabelas, colunas, payloads, eventos, incluindo chaves de documentos jsonb) **não** são renomeados mecanicamente — cada caso recebe estratégia própria de migração (ver Phase 1, STD-06, `persisted.csv`). Variáveis de ambiente mantêm nome (D-06/D-36). Inclui rotas/endpoints da API (não só front), e um contrato de navegação/renderização documentado (STD-12) — path params vs query params vs React state, não assumir SSR pela URL estável da Blip. Convenção canônica aprovada pelo dono no portão 1 (D-03) em 24/09/2026; mapa old→new por escopo é o portão 2.
...
| Linguagem técnica do projeto migra de português para inglês (rotas, endpoints, arquivos, funções, variáveis, types, testes, contrato de navegação/renderização), conforme `.planning/phases/01-padronizar-linguagem-t-cnica-navega-o-e-renderiza-o/std/CONVENTIONS-EN.md`; dados persistidos ficam fora do rename mecânico | Substitui a regra anterior "tudo em português"; texto visível ao usuário não muda. Tratado antes do Builder para não acumular mais superfície em português (Phase 1, STD-01..12) | ✓ Good — glossário e convenção aprovados pelo dono no portão 1 (D-03) em 24/09/2026; mapa old→new (portão 2) em andamento |
